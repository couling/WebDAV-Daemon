...
static void load_keys(const char *hostname, const char *CERT_FILE, const char *KEY_FILE) {
	int ret;
	gnutls_datum_t certData, keyData;
	struct Hosts *host = malloc(sizeof(struct Hosts));
	host->hostname = hostname;
	host->next = hosts;
	hosts = host;

	if ((ret = gnutls_load_file(CERT_FILE, &certData))
			|| (ret = gnutls_pcert_import_x509_raw(&host->pcrt, &certData, GNUTLS_X509_FMT_PEM, 0))
			|| (ret = gnutls_load_file(KEY_FILE, &keyData))
			|| (ret = gnutls_privkey_init(&host->key))
			|| (ret = gnutls_privkey_import_x509_raw(host->key, &keyData, GNUTLS_X509_FMT_PEM, NULL, 0))) {

		fprintf(stderr, "Error loading certificates and keys %s\n", gnutls_strerror(ret));
		exit(1);
	}
